            ...
            <div className="row">
                <div className="col-lg-12 col-md-12 col-sm-12 col-12">
                    <div className="vendor_order_boxed">
                        <div className='addres_list'><h4>Address List</h4>
                        <button type="submit" onClick={Addaddresscomp} className="theme-btn-one btn-black-overlay btn_sm">{"Add New"}</button>
                        </div>
                               {AddressList.map((data)=>
                                        <div className='card_content'>
                                            <div className='child_comp'>
                                            <div className='name_dispaly'><strong>{data.firstname} {data.lastname}</strong><span>{data.mobile}</span></div>
                                            <div><i class="fa fa-trash" aria-hidden="true" onClick={()=>DeleteAddress(data.id)}></i><i class="fa fa-pencil-square-o" aria-hidden="true" onClick={()=>EditAddress(data.id)}></i></div>
                                            </div>
